...
export const makeMarkers = (venues) => (
  venues.map((venue) => {
    return (
      <Marker position={[venue.lat, venue.lng]} id={venue.fs_venue_id} riseOnHover={true} mouseOver={() => this.openPopup()}>
        <Popup>
          <span>
            <h3>{venue.name}</h3>
            <p><strong>Address: </strong>{venue.address}</p>
            <p><strong>Website: </strong><a href={venue.url} target="_blank" rel="noopener noreferrer" >{venue.url}</a></p>
